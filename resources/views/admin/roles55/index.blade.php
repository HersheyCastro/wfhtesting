        ...
        <div class="col-md-12">
            ...
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title"><i class="fa fa-black-tie fa-fw"></i>Roles</h3>
                </div>
                <div class="box-body">

                    <table class="table table-striped table-hover table-responsive table-bordered" id="Roles55DataTable"
                           width="100%">
                        <thead>
                        <tr>
                            <th class="all">
                                ...
                                <div class="btn-group tools">
                                    @if(Guard::allows('roles55_create'))
                                        <button action="form" type="button"
                                                onclick="location.href ='{{ route('admin'.'.roles55.create') }}'"
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu pull-right ColumnToggle" role="menu">
                                            <li action="form" data-column="1" class="toggle-vis Checked"><a
                                                        href="javascript:void(0)"><i class="fa fa-check"></i>Active</a>
                                            </li>
                                            <li action="form" data-column="2" class="toggle-vis Checked"><a
                                                        href="javascript:void(0)"><i class="fa fa-check"></i>Role
                                                    Name</a></li>
                                            <li action="form" data-column="3" class="toggle-vis Checked"><a
                                                        href="javascript:void(0)"><i class="fa fa-check"></i>Permitted
                                                    Modules</a></li>
                        ...
                        <tbody>
                        @foreach ($roles55 as $row)
                            <tr>
                                <td>
                                    {!! Form::checkbox('del-'.encrypt($row->id),1,false,['class' => 'single','data-id'=> encrypt($row->id)]) !!}
                                </td>
                                @if($row->bActive== 1)
                                    <td><span class="label label-success">True</span></td>@else
                                    <td><span class="label label-danger">False</span></td>@endif
                                <td>{{ $row->sRoleName }}</td>
                                <td> @forelse($row->modules55 as $key) <span
                                            class="label label-primary">{{$key->modules55->sModuleName}}</span> @empty
                                        Empty @endforelse </td>
                                <td>
                                    ...
                                    <div class="btn-group tools">
                                        @if(Guard::allows('roles55_view'))
                                            <button type="button"
                                                    onclick="location.href ='{{route('admin'.'.roles55.show', array(encrypt($row->id))) }}'"
                                                    class="btn btn-default btn-sm fa fa-search"></button>
                                        @endif
                                        @if(Guard::allows('roles55_edit') || Guard::allows('roles55_delete'))
                                            <div class="btn-group">
                                                <button class="btn dropdown-toggle btn-default btn-sm fa fa-bars"
                    ...
                    {!! Form::open(['route' => 'admin'.'.roles55.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
...
@section('javascript')
    <script>
        $(document).ready(function () {
            ...
            var table = $('#Roles55DataTable').DataTable(
                {
                    "columnDefs": [{
                        "width": "30px",
                        "targets": 0,
