...
@section('title', 'Verify Accomplishment')
@section('content')
<style type="text/css">
    table#t01 th {
                  ...
                  @foreach($successindicators55->where('id',$target->successindicators55_id) as $si)
                    @foreach($strategicobjectives->where('id',$si->strategicobjectives55_id) as $so )
                      <tr>
                        ...
                        <td  rowspan="{{$tasks->where('targets_id', $target->id)->count() ?? '1'}}" style="">{!! $target->name !!}
                          <p style="color: red">({{$target->percent}}%)</p>
                        </td>
                        @endif
                        ...
                        <td>{{$task->senior_accomplishmentremarks}}</td>
                             ...
                             <!--  {!! Form::model($tasks, array('id' => 'form-with-validation', 'method' => 'PATCH', 'route' => array('admin'.'.tasks55.update', encrypt($task->tasks_id)))) !!}
                              ...
                              <!-- <td>
                                <button type="submit" class="btn btn-default btn-sm fa fa-pencil-square-o"></button>
                                {!! Form::submit('Updateeeee', array('class' => 'btn btn-primary')) !!}
                              </td> -->
                              <!-- {!! Form::close() !!} -->
                        ...
                        @if(!$counter)
                         <td rowspan="{{$tasks->where('targets_id', $target->id)->count() ?? '1'}}">
                              ...
                              <button type="button" onclick="location.href ='{{ route('admin'.'.editsenioraccomplishment',$target->id) }}'" class="btn btn-default btn-sm fa fa-pencil-square-o"></button>
